...
from models.text_utils import TextEncoder
...
BPE_PATH = os.path.join(MODELS_PATH, "vocab_40000.bpe")
...
((trX1, trX2, trX3, trY),
 (vaX1, vaX2, vaX3, vaY),
 (teX1, teX2, teX3)) = encode_dataset(*rocstories(DATA_DIR, n_valid=n_valid),
...
train = TensorDataset(torch.from_numpy(trX).to(devices[0]),
                     )
valid = TensorDataset(torch.from_numpy(trX).to(devices[0]))
train_loader = DataLoader(train, batch_size=n_batch, shuffle=True)
valid_loader = DataLoader(train, batch_size=n_batch, shuffle=True)
#if submit:
